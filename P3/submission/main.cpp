...
if (input.is_key_down(GLFW_KEY_I)) {
    ...
    } else sceneC.getActive()->translateScene(Vector<3>{forward[0] * camSpeed, forward[1] * camSpeed, forward[2] * camSpeed});
}
...
if (input.is_key_down(GLFW_KEY_J)) {
    ...
    } else sceneC.getActive()->translateScene(Vector<3>{right[0] * camSpeed, right[1] * camSpeed, right[2] * camSpeed});
}
...
if (input.is_key_down(GLFW_KEY_U)) {
    if (droneMode) cam.position[1] += camSpeed;
    else sceneC.getActive()->translateScene(Vector<3>{0.f, -camSpeed, 0.f});
}
