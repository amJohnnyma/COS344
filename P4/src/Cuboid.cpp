...
template <int n>
    ...
    if (m_splitLevel > 0)
        applySplits(m_splitLevel);

    setColor(this->color[0], this->color[1], this->color[2], this->color[3]);
    }
// Internal version that just applies splits without touching m_splitLevel
template <int n>
void Cuboid<n>::applySplits(int levels)
{
    for (int l = 0; l < levels; ++l)
    {
        struct Tri { Vector<n> a, b, c; };
        Tri tris[childCount];

        for (int i = 0; i < childCount; ++i)
        {
            Triangle<n>* tri = dynamic_cast<Triangle<n>*>(children[i]);
            tris[i] = { tri->getA(), tri->getB(), tri->getC() };
        }

        int triCount = childCount;
        for (int i = 0; i < childCount; ++i) delete children[i];
        childCount = 0;

        for (int t = 0; t < triCount; ++t)
        {
            const Vector<n>& a = tris[t].a;
            const Vector<n>& b = tris[t].b;
            const Vector<n>& c = tris[t].c;

            Vector<n> mAB, mBC, mAC;
            for (int k = 0; k < n; ++k) {
                mAB[k] = (a[k] + b[k]) * 0.5f;
                mBC[k] = (b[k] + c[k]) * 0.5f;
                mAC[k] = (a[k] + c[k]) * 0.5f;
            ...
            add(new Triangle<n>(a,   mAB, mAC));
            add(new Triangle<n>(mAB, b,   mBC));
            add(new Triangle<n>(mAC, mBC, c  ));
            add(new Triangle<n>(mAB, mBC, mAC));
        }
    }
}

template <int n>
void Cuboid<n>::splitVertices(int levels)
{
    if (levels < 0) {
        m_splitLevel = 0;
        rebuild();
    ...
    m_splitLevel += levels;
    applySplits(levels);  // only apply the NEW levels on top of current state
