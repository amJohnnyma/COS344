...
if [[ "$ACTION" == "create" ]]; then
    ...
    cp main.cpp                  "$SUBMIT/"
    cp windmillPlan.png.png                  "$SUBMIT/"
    ...
    cp src/Cone.cpp                "$SUBMIT/"
    cp src/Cuboid.cpp                "$SUBMIT/"
    cp src/Cylinder.cpp                "$SUBMIT/"
    cp src/Sphere.cpp                "$SUBMIT/"
    cp src/Triangular_Prism.cpp                "$SUBMIT/"
    ...
    cp include/math/Cone.h             "$SUBMIT/math_Cone.h"
    cp include/math/Cuboid.h             "$SUBMIT/math_Cuboid.h"
    cp include/math/Cylinder.h             "$SUBMIT/math_Cylinder.h"
    cp include/math/Matrix.h             "$SUBMIT/math_Matrix.h"
    cp include/math/Shape.h              "$SUBMIT/math_Shape.h"
    cp include/math/Sphere.h              "$SUBMIT/math_Sphere.h"
    cp include/math/Square.h             "$SUBMIT/math_Square.h"
    cp include/math/Triangle.h           "$SUBMIT/math_Triangle.h"
    cp include/math/Triangular_Prism.h           "$SUBMIT/math_Triangular_Prism.h"
...
if [[ "$ACTION" == "makerun" ]]; then
    ...
    for f in Vector.cpp Square.cpp Matrix.cpp Triangle.cpp Circle.cpp \
              ...
              Camera.cpp FpsLimiter.cpp Shape.cpp PhysicsBody.cpp Cone.cpp Cuboid.cpp Cylinder.cpp Sphere.cpp Triangular_Prism.cpp; do
        [[ -f "$f" ]] && mv "$f" src/
    done
    ...
    [[ -f math_Cone.h ]]         && mv math_Cone.h         include/math/Cone.h
    [[ -f math_Cuboid.h ]]         && mv math_Cuboid.h         include/math/Cuboid.h
    [[ -f math_Cylinder.h ]]         && mv math_Cylinder.h         include/math/Cylinder.h
    [[ -f math_Sphere.h ]]         && mv math_Sphere.h         include/math/Sphere.h
    [[ -f math_Triangular_Prism.h ]]         && mv math_Triangular_Prism.h         include/math/Triangular_Prism.h
