...
enum class TextureMode { Checker, DimpleColor, Displacement, Alpha };
enum TextureBit {
    TEX_CHECKER     = 1 << 0,
    TEX_DIMPLE      = 1 << 1,
    TEX_DISPLACEMENT= 1 << 2,
    TEX_ALPHA       = 1 << 3,
};
...
class Renderer {
    private:
        ...
        GLint uTextureLoc;
        GLuint m_dimpleColorTexture;
        GLuint m_dimpleDisplacementTexture;
        GLuint m_dimpleAlphaTexture;
        GLuint m_activeTexture;  // points to whichever is active
        GLint uDimpleColorLoc;
        GLint uDisplacementLoc;
        GLint uAlphaLoc;
...
void setTextureMask(int mask);
void setDisplaceStrength(float s);
    private:
        void loadShaders();
        ...
        GLuint createDimpleColorTexture();
        GLuint createDimpleDisplacementTexture();
        GLuint createDimpleAlphaTexture();
